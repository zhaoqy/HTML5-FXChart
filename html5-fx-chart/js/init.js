...
//init load data
addEventListener('load', function(){init();} ,false);
//Change orientation listener
parent.window.addEventListener('orientationchange', function(){init();} ,false);
function init(){
...
function showSetting(){
	if(CHART_SETTING != null){
		...
		if(CHART_SETTING[3]==0 || CHART_SETTING[3]==4){$("set1").disabled=true;}else{$("set1").disabled=false;}
		if(CHART_SETTING[4]==0){$("set2").disabled=true;}else{$("set2").disabled=false;}
	}
...
function setPosition(){
	...
	if (innerWidth != appliWidth) {
        ...
        if(appliWidth > 321) { init_chart_settings(true); } else { init_chart_settings(false);}
	}
...
function init_chart_settings(ver_hor){
	if(ver_hor){   //horizontal
		...
		max = 105;
		min = 45;
		for(var i in canvas){canvas[i].width=410;canvas[i].height=200;}
		...
		max = 65;
		min = 25;
		for(var i in canvas){canvas[i].width=325;canvas[i].height=328;}
		CHART['canvas_w'] = ctx_chart.width;
		CHART['canvas_h'] = ctx_chart.height;		
		CHART['x_start'] = 0;
		CHART['y_start'] = 12;
	...
	if(v_pos<chart_settings['x_pos']){
		...
		if(utimer!=''){
			if(type_settings['unit']== "day"){
				...
				var Months = time[1],Dates = time[2];
