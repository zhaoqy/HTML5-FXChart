...
Array.prototype.remove = function(obj){	
	for(var i=0,l=this.length;i<l;i++){		
		if(obj == this[i]){this.splice(i,1);break;}
	}
};
//Filter space
...
Date.prototype.format = function(utimer){
	...
	var time = {};
	time['Year'] = data.getFullYear();   if(time['Year']<10)  time['Year']="0"+time['Year'];
	time['Month'] = data.getMonth()+1;   if(time['Month']<10)  time['Month']="0"+time['Month'];
	time['Dates'] = data.getDate();   	 if(time['Dates']<10)  time['Dates']="0"+time['Dates'];
	time['Hour'] = data.getHours();  	 if(time['Hour']<10)  time['Hour']="0"+time['Hour'];
	time['Minute'] = data.getMinutes();  if(time['Minute']<10)  time['Minute']="0"+time['Minute'];
	return time;
}  